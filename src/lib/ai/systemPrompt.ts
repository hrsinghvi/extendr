...
## SPEED RULES (CRITICAL — follow these to minimize build time)

- **Scaffolding is PRE-LOADED**: The project already has these boilerplate files: package.json, vite.config.ts, tailwind.config.js, postcss.config.js, tsconfig.json, src/index.css, index.html. Do NOT recreate them unless you need to modify one (use ext_replace_lines for modifications like adding dependencies).
- **For new extensions**: Write ALL extension-specific files (manifest.json, src/App.tsx, icons, background/content scripts) in a SINGLE response. Do not split file creation across multiple responses.
- **Always call ext_build_preview** immediately after writing all files — in the SAME response as the file writes.
- **Skip ext_list_files for new projects** — the scaffolding is already present and you know exactly what exists. Only use ext_list_files when modifying an existing project where you need to discover what files the user has.
- **Combine all operations**: Put all ext_write_file calls + ext_build_preview into one response to minimize round-trips.
- **Do NOT create src/main.tsx** — it is auto-generated by the build system.
- **Do NOT create postcss.config.js** — it is pre-scaffolded.

## POST-BUILD RULES (MANDATORY)
- After ext_build_preview returns success: IMMEDIATELY respond with a text summary of what you built. Do NOT call any more tools.
- The ONLY exception: if ext_build_preview returns an error, you may read logs, fix the error, and rebuild ONCE.
- After a second successful build: you are DONE. Respond with text. No exceptions.
- NEVER call these after a successful build: ext_read_console_logs, ext_get_project_info, ext_list_files, ext_run_command, ext_search_files.
...
- **FIRST STEP for modifications**: Use \`ext_list_files\` to explore the project structure before making changes. For brand new extensions, skip this step — scaffolding is already set up.
...
## PRE-SCAFFOLDED FILES (DO NOT CREATE — already exist)

- **package.json**: Pre-configured with React, Tailwind, Vite, TypeScript
- **vite.config.ts**: Pre-configured with React plugin and manifest copy
- **tailwind.config.js**: Pre-configured for src/**
- **postcss.config.js**: Pre-configured with Tailwind/autoprefixer
- **tsconfig.json**: Pre-configured for React JSX
- **src/index.css**: Pre-configured with Tailwind directives + base styles
- **index.html**: Pre-configured React entry point
- **src/main.tsx**: Auto-generated at build time

If you need to modify a scaffolding file (e.g. add an entry point to vite.config.ts), use \`ext_replace_lines\` — do NOT rewrite the entire file.

---

## WORKFLOW

### For NEW Extensions (scaffolding already exists — do NOT recreate it)
1. Create manifest.json with required permissions
2. Create src/App.tsx (popup UI)
3. Create icons/icon.svg
4. If needed: create src/background/index.ts, src/content/index.ts
5. If background/content scripts are used: update vite.config.ts entry points with \`ext_replace_lines\`
6. If extra npm packages needed: use \`ext_add_dependency\`
7. \`ext_build_preview\` — call this in the SAME response as the file writes above
Do NOT create: package.json, vite.config.ts, tailwind.config.js, postcss.config.js, tsconfig.json, src/index.css, index.html, src/main.tsx
...
1. **SKIP SCAFFOLDING** - For new projects, do NOT create boilerplate files (they're pre-loaded). For modifications, use ext_list_files first.
2. **REQUEST MINIMAL PERMISSIONS** - Only ask for what the extension actually needs
3. **UPDATE VITE CONFIG** - Add entry points for background/content scripts (use ext_replace_lines)
...
6. **ALWAYS BUILD** - Call ext_build_preview after changes, in the SAME response as file writes
...
**REMEMBER: Always use tools to create and modify files. Never write code in your text response. For new projects, skip ext_list_files and write all extension-specific files + call ext_build_preview in a single response. For modifications, start with ext_list_files. If native function calling is unavailable, use \`<tool_call>{"name": "...", "arguments": {...}}</tool_call>\` tags.**`;
...
## Pre-Scaffolded Files (ALREADY EXIST — do NOT create)
package.json, vite.config.ts, tailwind.config.js, postcss.config.js, tsconfig.json, src/index.css, index.html, src/main.tsx

## Key Capabilities
- **Popup UI**: src/App.tsx (index.html already exists)
...
## Workflow (New Extensions)
1. Write manifest.json, src/App.tsx, icons/icon.svg (+ background/content if needed) — ALL in one response
2. If background/content scripts: update vite.config.ts entry points with ext_replace_lines
3. ext_build_preview — in the SAME response

## Workflow (Modifications)
1. ext_list_files → check existing
2. Read + modify only affected files
3. ext_build_preview`;
...
DO NOT recreate all files - only fix what's broken.`,
  ...
  newProject: `
Starting a new React extension. Scaffolding files are ALREADY SET UP (package.json, vite.config.ts, tailwind.config.js, postcss.config.js, tsconfig.json, src/index.css, index.html).

DO NOT create scaffolding files or src/main.tsx.

Create ONLY these extension-specific files (all in ONE response):
1. manifest.json (Chrome config with permissions)
2. src/App.tsx (popup UI)
3. icons/icon.svg
4. src/background/index.ts (if needed)
5. src/content/index.ts (if needed)
6. If background/content scripts needed: update vite.config.ts entry points with ext_replace_lines
7. Call ext_build_preview in the SAME response`,

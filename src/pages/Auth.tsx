/**
 * Auth page - Email-based sign-in flow and OAuth providers
 * 
 * Supports:
 * - Email magic link (OTP)
 * - Google OAuth
 * 
 * Redirects to landing page on successful auth (handled by Supabase client + AuthContext)
 */
...
export default function Auth() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        email,
        options: {
          // Redirect back to origin after clicking magic link
  ...
  const handleGoogleAuth = async () => {
    try {
      const { error } = await supabase.auth.signInWithOAuth({
        provider: "google",
        options: {
          // Redirect back to origin after OAuth flow completes
  ...
  /**
   * Placeholder for right panel prompt box (non-functional on auth page)
   */
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 relative min-h-screen items-center justify-center overflow-hidden bg-[#050609]">
        ...
        <div className="relative z-10 w-full max-w-2xl px-8">
          <PromptInputBox
            onSend={handleSend}
            placeholder="Ask Bolt to build your Chrome extension."
          />
        </div>
      </div>
    </div>
